    ...
    <v-main class="mt-12 container-global ma-auto">
      <v-container class="full-height d-flex justify-center flex-wrap align-center" fluid>
        <v-col cols="12" md="6" class="mt-6">
          <v-img
            v-if="this.data.publication.imgages_path != ''"
            :src="'/uploads/images/'+JSON.parse(data.publication.imgages_path)"
            max-height="500px"
            class="rounded-lg"
          >
          </v-img>
        ...
        <v-col cols="12" md="6">
          <div class="black--text text-capitalize text-left text-h5 font-weight-bold mt-6">{{data.user.name}} - {{data.user.years}}</div>
          <div class="grey--text darken-2 text-capitalize text-left text-subtitle-2 font-weight-light mb-6">Miembro desde {{moment(data.user.created_at).startOf('hour').format('DD-MMMM-YYYY')}}</div>
          ...
          <v-col cols="12" class="d-flex justify-end">
            ...
            <v-btn @click="redirectWhatsapp(data.user.whatsapp)" class="ml-4" color="success">Whatsapp</v-btn>
          </v-col>

        </v-col>
        <v-row style="width: 100%" >
          <table-data :userDataTable="data.user" :times="data.times" style="width: 100%" ></table-data>
        </v-row>
        <v-row style="width: 100%" >
          <v-col cols="12">
